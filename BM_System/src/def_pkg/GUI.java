...
public class GUI {
	...
	public Image getScaledImage(Image srcImg, int maxWidth, int maxHeight) {
		...
		double scale = Math.min(widthRatio, heightRatio); // scale proportionally
	...
	public void openSignInForm(JFrame frame, Login_Account user) {
		...
		ImageIcon originalIcon = new ImageIcon(getClass().getResource("/def_pkg/iiita_bank.png"));
		Image scaled = getScaledImage(originalIcon.getImage(), 450, 400);
		ImageIcon resizedIcon = new ImageIcon(scaled);
		
		JLabel imgLabel = new JLabel(resizedIcon);
		imgLabel.setBounds(40, 30, resizedIcon.getIconWidth(), resizedIcon.getIconHeight());
		f.add(imgLabel);
		
		
		//---------------------------------Creating Labels------------------------------------------		
		JLabel l_hSignIn = new JLabel("IIITA BANK");
		l_hSignIn.setFont( l_hSignIn.getFont().deriveFont(30f) );
		l_hSignIn.setBounds(500,50,200, 40);
		f.add(l_hSignIn);
